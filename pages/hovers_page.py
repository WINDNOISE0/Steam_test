...
class HoversPage(BasePage):
    ...
    USER_CARD_LOC_FMT = "(//img[@alt='User Avatar'])[{}]"

    VIEW_PROFILE_BUTTON_LOC_FMT = "//a[@href='/users/{}']"
    USER_CARD_DATA_LOC_FMT = "//div[contains(@class, 'figcaption')]/*[contains(text(), 'name: user{}')]"

    def __init__(self, browser):
        super().__init__(browser)
        self.unique_element = WebElement(browser, self.UNIQUE_ELEMENT_LOC)
        self.users_cards = MultiWebElement(browser, self.USER_CARS_LOC)

    def hover_random_user(self, user_number):
        user_card = WebElement(self.browser, self.USER_CARD_LOC_FMT.format(user_number))
        user_card.move_to_element()

    def click_view_profile(self, user_number):
        view_profile_button = Button(self.browser, self.VIEW_PROFILE_BUTTON_LOC_FMT.format(user_number))
        view_profile_button.click()

    def get_actual_user_number(self, user_number):
        user_card_data = Label(self.browser, self.USER_CARD_DATA_LOC_FMT.format(user_number))
